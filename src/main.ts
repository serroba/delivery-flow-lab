...
const presets: WorkflowPreset[] = [
  {
    ...
    description:
      "A relatively healthy flow where code review and release are both active constraints.",
    stages: [
      {
        id: "shape",
        name: "Shape",
        role: "Scoping work for the next move",
        serviceHours: 1.2,
        servers: 2,
        touchMultiplier: 1,
        arrivalCv2: 1,
      ...
      },
      {
        id: "build",
        name: "Build",
        role: "Implementation and local verification",
        serviceHours: 5.5,
        servers: 5,
        touchMultiplier: 1,
        arrivalCv2: 1.1,
      ...
      },
      {
        id: "review",
        name: "Review",
        role: "PR review, revisions, and merge decisions",
        serviceHours: 1.8,
        servers: 2,
      ...
      },
      {
        id: "release",
        name: "Release",
        role: "CI, approvals, deployment windows, rollback checks",
        serviceHours: 1.5,
        servers: 1,
  ...
  },
  {
    ...
    name: "Review Drag",
    description:
      "Teams are shipping often, but review depth and rework are eating cycle time.",
    stages: [
      {
        id: "shape",
        name: "Shape",
        role: "Clarify intent and split work",
        serviceHours: 1,
        servers: 2,
        touchMultiplier: 1,
        arrivalCv2: 1,
      ...
      },
      {
        id: "review",
        name: "Review",
        role: "Deep review and re-review loops",
        serviceHours: 2.5,
        servers: 2,
  ...
  },
  {
    ...
    description:
      "Engineering can finish work, but deploy gates and release handling create the real queue.",
    stages: [
      {
        id: "shape",
        name: "Shape",
        role: "Scope and acceptance",
        serviceHours: 0.9,
        servers: 2,
        touchMultiplier: 1,
        arrivalCv2: 1,
      ...
      },
      {
        id: "release",
        name: "Release",
        role: "Release train, approvals, post-deploy checks",
        serviceHours: 2.4,
        servers: 1,
...
function bottleneckSummary(
  ...
  if (unstableStage) {
    return {
      ...
      detail:
        "That queue is receiving work faster than it can clear it. Reduce concurrent work, add capacity, or lower touch frequency before optimizing anywhere else.",
    };
  }
  ...
  return {
    ...
    detail:
      "Most waiting time is accumulating here, so this is where attention will buy down cycle time first.",
  };
}

function recommendationLines(allMetrics: StageMetrics[]): string[] {
  const paired = stages.map((stage, index) => ({ stage, metrics: allMetrics[index] }));
  const hottest = [...paired].sort(
    (left, right) => right.metrics.utilization - left.metrics.utilization,
  )[0];
  const safeLimit = Math.min(
    ...stages.map((stage) => safeConcurrencyFor(stage, state)),
  );
  ...
  if (hottest.stage.id === "review") {
    lines.push(
      "Focus on code review policy, review staffing, or smaller changesets before increasing throughput.",
    );
  } else if (hottest.stage.id === "release") {
    lines.push(
      "Unblock release handling first. Faster coding will mostly create a larger ready-to-ship pile.",
    );
  } else {
    lines.push(
      `The next improvement is upstream in ${hottest.stage.name.toLowerCase()}, not in downstream release operations.`,
    );
  }

  if (state.activeWorkstreams > roundedSafeLimit) {
    lines.push(
      `Current workstream count is above the model's comfortable limit of about ${roundedSafeLimit}. Expect queues to grow quickly.`,
    );
  } else {
    lines.push(
      `You can likely run about ${roundedSafeLimit} concurrent workstreams before any stage crosses the ${Math.round(state.targetUtilization * 100)}% risk line.`,
    );
  }
  ...
  if (Number.isFinite(totalCycle) && totalCycle > 0) {
    lines.push(
      `${Math.round((totalWait / totalCycle) * 100)}% of total flow time is queueing rather than hands-on work in this scenario.`,
    );
  }
...
function buildStageCard(stage: Stage, metrics: StageMetrics, index: number): string {
  ...
  const safeConcurrency = Math.max(
    1,
    Math.floor(safeConcurrencyFor(stage, state)),
  );
...
function render(): void {
  const allMetrics = stages.map((stage) => computeStageMetrics(stage, state));
  const flowSummary = bottleneckSummary(allMetrics);
  const totalCycle = allMetrics.reduce((sum, metrics) => sum + metrics.cycleHours, 0);
  const totalQueue = allMetrics.reduce((sum, metrics) => sum + metrics.queueHours, 0);
  const workstreamLimit = Math.max(
    1,
    Math.floor(Math.min(...stages.map((stage) => safeConcurrencyFor(stage, state)))),
  );
